...
const TrendingTable = () => {
  return (
    <div className="py-16">
      <h2 className="text-white text-[24px] mb-6">Trending Coins</h2>
          ...
          <tbody>
            {tableData.map((row, idx) => (
              <tr key={idx} className="border-b border-gray-700">
                <td className="px-4 py-3"><div className='flex gap-3'><img src={row.src} className='w-[28px] h-[28px]' alt="" />{row.coin}</div></td>
                <td className="px-4 py-3">{row.price}</td>
                <td className={`px-4 py-3 font-medium ${row.changeColor}`}>
                  {row.change}
                </td>
                <td className="px-4 py-3">{row.turnover}</td>
                <td className="text-right px-4 py-3 text-[#913DFF] cursor-pointer hover:underline">
                  Trade
                </td>
              </tr>
            ))}
      ...
      <Link to='/spot'>
      <p className="text-center underline text-gray-500 mt-4 cursor-pointer hover:text-gray-300">
        View All Coins
      </p>
      </Link>
